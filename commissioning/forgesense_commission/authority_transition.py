...
def _require_policy_id(value: object) -> str:
    ...
    if not re.fullmatch(r"[A-Za-z0-9._-]{1,96}", text):
        raise MaintenanceAuthorityTransitionError("transition policy_id is invalid")
    return text
...
def _validate_policy(policy: dict[str, Any]) -> None:
    ...
    _require_policy_id(policy.get("policy_id"))
    ...
    expected_binding = {
        ...
        "audit_ledger_head_required": True,
        "calibration_sequence_unchanged_required": True,
        "active_record_sha256_unchanged_required": True,
        "old_public_key_fingerprint_required": True,
        "new_public_key_fingerprint_required": True,
        "source_commit_required": True,
        "source_checkout_head_must_equal_source_commit": True,
        "maintenance_source_clean_required": True,
        "maintenance_source_manifest_root_required": True,
        "sdkconfig_sha256_required": True,
        "maintenance_image_sha256_required": True,
        "sdkconfig_must_pin_new_public_key": True,
    }
    ...
    for section, expected, label in (
        (signatures, expected_signatures, "signature"),
        (binding, expected_binding, "binding"),
        (installation, expected_installation, "installation"),
        (audit, expected_audit, "audit"),
        (authority, expected_authority, "authority"),
    ):
        for key, expected_value in expected.items():
            if section.get(key) != expected_value:
                raise MaintenanceAuthorityTransitionError(
                    f"transition {label} policy field {key} is invalid"
                )
...
def _git_output(repo_root: Path, args: list[str]) -> str:
    try:
        ...
        raise MaintenanceAuthorityTransitionError(
            "Git is required to bind maintenance sources to a reviewed commit"
        ) from exc
...
def _device_mac(device_id: str) -> bytes:
    if not re.fullmatch(r"esp32s3:[0-9a-f]{12}", device_id):
        raise MaintenanceAuthorityTransitionError(
            "device_id must use esp32s3:<12 lowercase hex> format"
        )
...
def _source_manifest(repo_root: Path, source_commit: str) -> dict[str, Any]:
    ...
    for relative in _SOURCE_FILES:
        ...
        try:
            ...
            raise MaintenanceAuthorityTransitionError(
                "maintenance source path escapes repository root"
            ) from exc
        if path.is_symlink() or not path.is_file():
            raise MaintenanceAuthorityTransitionError(
                f"maintenance source is not a regular file: {relative}"
            )
        files.append({"path": relative, "sha256": _file_sha256(path)})
    material = {"source_commit": source_commit, "files": files}
    return {
        "schema": SOURCE_MANIFEST_SCHEMA,
        "source_commit": source_commit,
        "files": files,
        "root_sha256": _canonical_sha256(material),
    }


def _validate_source_manifest(manifest: dict[str, Any], source_commit: str) -> str:
    ...
    if not isinstance(files, list) or len(files) != len(_SOURCE_FILES):
        raise MaintenanceAuthorityTransitionError("transition source-manifest file set is incomplete")
    observed_paths: list[str] = []
    for item in files:
        if not isinstance(item, dict):
            raise MaintenanceAuthorityTransitionError("transition source-manifest file entry is invalid")
        path = str(item.get("path", ""))
        _require_sha256(item.get("sha256"), f"source manifest SHA-256 for {path}")
        observed_paths.append(path)
    if observed_paths != list(_SOURCE_FILES):
        raise MaintenanceAuthorityTransitionError("transition source-manifest path set/order is invalid")
    material = {"source_commit": source_commit, "files": files}
    root = _canonical_sha256(material)
    if manifest.get("root_sha256") != root:
...
def transition_payload(
    *,
    ...
    policy = _require_policy_id(policy_id).encode("ascii")
...
def _payload_from_request(request: dict[str, Any], manifest: dict[str, Any]) -> bytes:
    source_commit = _full_commit(str(request.get("source_commit", "")))
    manifest_root = _validate_source_manifest(manifest, source_commit)
    if request.get("source_manifest_root_sha256") != manifest_root:
...
def _device_state_matches_ledger(device: dict[str, Any], ledger: LedgerState) -> None:
    if str(device.get("device_id", "")) != ledger.device_id:
        raise MaintenanceAuthorityTransitionError(
            "device state belongs to a different audit ledger device"
        )
    if int(device.get("installed_sequence", -1)) != ledger.sequence:
        raise MaintenanceAuthorityTransitionError(
            "device calibration sequence differs from audit ledger head"
        )
    record_sha = _require_sha256(
        device.get("active_record_sha256"), "device active record SHA-256", allow_none=True
    )
    if record_sha != ledger.record_sha256:
        raise MaintenanceAuthorityTransitionError(
            "device active record SHA-256 differs from audit ledger head"
        )
    ...
    if authority_sha != ledger.authority_public_key_sha256:
        raise MaintenanceAuthorityTransitionError(
            "device maintenance authority fingerprint differs from audit ledger head"
        )
...
def build_transition_request(
    *,
    ...
    policy_id = _require_policy_id(policy["policy_id"])
    ...
    if old_fp != ledger.authority_public_key_sha256:
        raise MaintenanceAuthorityTransitionError(
            "old public key does not match current audit-ledger authority"
        )
    if new_fp == old_fp:
        raise MaintenanceAuthorityTransitionError(
    ...
    if match is None:
        raise MaintenanceAuthorityTransitionError(
            "sdkconfig does not contain the maintenance authority DER pin"
        )
...
def package_transition(
    ...
    *,
    ...
    if request.get("source_checkout_verified_clean") is not True:
        raise MaintenanceAuthorityTransitionError(
            "transition request lacks verified clean source-checkout binding"
        )
    manifest_obj = _load_json(request_dir / "source-manifest.json", "maintenance source manifest")
    payload = (request_dir / "transition-payload.bin").read_bytes()
    expected_payload = _payload_from_request(request, manifest_obj)
    if payload != expected_payload or request.get("payload_sha256") != _sha256_bytes(payload):
        raise MaintenanceAuthorityTransitionError("transition request payload does not match bound metadata")

    old_signature = old_signature_path.read_bytes()
    new_signature = new_signature_path.read_bytes()
    old_fp = verify_signature_openssl(
        public_key_path=old_public_key_path, payload=payload, signature_der=old_signature
    )
    new_fp = verify_signature_openssl(
        public_key_path=new_public_key_path, payload=payload, signature_der=new_signature
    )
    if old_fp != request.get("old_authority_public_key_sha256"):
        raise MaintenanceAuthorityTransitionError(
            "old transition signer fingerprint differs from request"
        )
    if new_fp != request.get("new_authority_public_key_sha256"):
        raise MaintenanceAuthorityTransitionError(
            "new transition signer fingerprint differs from request"
        )

    output_dir = output_dir.resolve()
    if output_dir.exists():
        raise MaintenanceAuthorityTransitionError(
            f"transition package output already exists: {output_dir}"
        )
    ...
    try:
        ...
        (staging / "source-manifest.json").write_bytes(_json_bytes(manifest_obj))
...
def verify_transition_package(package_dir: Path) -> VerifiedAuthorityTransition:
    ...
    if package.get("schema") != TRANSITION_SCHEMA or request.get("schema") != TRANSITION_REQUEST_SCHEMA:
        raise MaintenanceAuthorityTransitionError(
            "unsupported maintenance-authority transition package schema"
        )
    if request.get("source_checkout_verified_clean") is not True:
        raise MaintenanceAuthorityTransitionError(
            "transition request lacks verified clean source-checkout binding"
    ...
    expected_payload = _payload_from_request(request, manifest)
    if payload != expected_payload or request.get("payload_sha256") != _sha256_bytes(payload):
    ...
    old_fp = verify_signature_openssl(
        public_key_path=old_public, payload=payload, signature_der=old_signature
    )
    new_fp = verify_signature_openssl(
        public_key_path=new_public, payload=payload, signature_der=new_signature
    )
    if old_fp != request.get("old_authority_public_key_sha256") or new_fp != request.get(
        "new_authority_public_key_sha256"
    ):
    ...
    for key, expected in expected_package.items():
        if package.get(key) != expected:
            raise MaintenanceAuthorityTransitionError(
                f"transition package metadata mismatch for {key}"
            )
    ...
    for key, expected in expected_authority.items():
        if authority.get(key) != expected:
            raise MaintenanceAuthorityTransitionError(
                f"transition package authority field {key} is invalid"
            )
