...
- [`CALIBRATION_RECOVERY.md`](CALIBRATION_RECOVERY.md) — restore an earlier approved coefficient set only through a new higher sequence, exact active-record binding, signed maintenance authorization, no-op rejection, and source-review-only maintenance-authority key rotation.
- [`CALIBRATION_AUDIT_LEDGER.md`](CALIBRATION_AUDIT_LEDGER.md) — maintain a per-device append-structured SHA-256 calibration history, require live exact-record/signer continuity before writes, bind write/recovery/reboot evidence, and fail closed on tamper or history gaps.
