...
- [`CALIBRATION_MAINTENANCE_PROVISIONING.md`](CALIBRATION_MAINTENANCE_PROVISIONING.md) — use a separate default-disabled maintenance image for read-only device-state capture, dual physically gated PREPARE/COMMIT, exact readback, retained write evidence, and manual reboot recovery verification.
