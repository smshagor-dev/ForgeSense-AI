...
from forgesense_protocol import (
    FreshnessGate,
    HealthClass as ProtocolHealthClass,
    MlObservation,
    VALID_OBSERVATION,
    decode_frame,
...
def main() -> int:
    ...
    for seq, sample in enumerate(scenario):
        if sample.sample_index < STARTUP_SETTLE_SAMPLES or not sample.all_valid:
            continue
        ...
        packet = encode_ml_observation(
            observation,
            sequence=seq & 0xFFFF,
            timestamp_ms=round(sample.time_s * 1000) & 0xFFFFFFFF,
        )
    ...
    final_score = model.infer(scenario[-1].feature_vector()).anomaly_score
    print(
        {
            "scenario": "bearing_degradation",
            "samples": len(scenario),
            "accepted_observations": accepted,
            "first_critical_sample": first_critical,
            "final_anomaly_score": round(final_score, 5),
        }
    )
