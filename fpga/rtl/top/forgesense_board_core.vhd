...
architecture rtl of forgesense_board_core is
    function watchdog_cycles(freq : positive; timeout_ms : positive) return positive is
    ...
    constant WATCHDOG_CYCLES : positive := watchdog_cycles(CLK_FREQ_HZ, WATCHDOG_TIMEOUT_MS);
...
begin
    sample_clock : entity work.sample_scheduler
        ...
        port map (clk => clk, rst => rst, sample_tick => sample_tick_i);

    status_clock : entity work.sample_scheduler
        generic map (CLK_FREQ_HZ => CLK_FREQ_HZ, SAMPLE_RATE_HZ => STATUS_RATE_HZ)
        port map (clk => clk, rst => rst, sample_tick => status_tick_i);
    ...
    serial_rx : entity work.uart_rx
        ...
        port map (
            ...
            data_valid => rx_valid_i, data_byte => rx_byte_i,
    ...
    serial_tx : entity work.uart_tx
        generic map (CLK_FREQ_HZ => CLK_FREQ_HZ, BAUD_RATE => UART_BAUD_RATE)
        port map (
            clk => clk, rst => rst, data_valid => tx_valid_i,
            data_byte => tx_byte_i, ready => tx_ready_i, tx_serial => uart_tx_o
        );

    platform : entity work.forgesense_platform_core
        generic map (WATCHDOG_TIMEOUT_CYCLES => WATCHDOG_CYCLES)
        port map (
            clk => clk, rst => rst, sample_tick => sample_tick_i,
            status_tick => status_tick_i, monotonic_ms => monotonic_ms_i,
