...
Hardware implementation artifacts live under `hardware/`. The active electrical sources include `profiles/power_entry_v1.json`, `profiles/sensor_support_v1.json`, `profiles/sensor_devices_v1.json`, `profiles/interconnect_v1.json`, the preliminary BOM, the machine-readable schematic contract, the package/MPN manifest, the net-endpoint freeze table, the `hardware/bringup/` harness/test/evidence contracts, and the `hardware/calibration/` diagnostic/session/capture/review/campaign/approval/source-change/provisioning/audit/authority-transition contracts. `make hardware-check` verifies cross-file consistency and analytical constraints; `make bringup-check` verifies physical bring-up; `make commissioning-check` verifies the commissioning bridge/reporting contract; `make calibration-diagnostic-check` verifies the read-only physical diagnostic path; `make calibration-assembler-check` verifies trusted diagnostic + independent-reference assembly; `make calibration-check` verifies single-run proposals plus repeated-run uncertainty/repeatability review; `make calibration-campaign-check` verifies repeated-run orchestration and evidence-index integrity; `make calibration-bundle-verification-check` verifies campaign tamper detection, original-source provenance, reviewer gating, and no-private-key external-signing boundaries; `make calibration-source-change-check` verifies explicit approval binding, integer quantization, add-only profile patching, and hard-safety non-regression; `make calibration-provisioning-check` verifies exact record encoding, strict sequence ordering, device-state binding, and dual-slot storage semantics; `make maintenance-provisioning-check` verifies the default-disabled maintenance image, read-only device-state capture, dual physical gates, challenged signed write, retained evidence, and production separation; `make signed-maintenance-authorization-check` verifies deterministic authorization binding and pinned-key verification; `make calibration-recovery-check` verifies exact active-record next-sequence restoration; `make calibration-audit-ledger-check` verifies fresh-device ledger genesis, exact write/recovery/reboot continuity, and explicit authority transitions; `make maintenance-authority-transition-check` verifies old/new signatures over one transition payload, source/config/image binding, unchanged calibration state, and the absence of runtime/remote key-write authority. Calibration tooling does not authorize actuator control or hard-safety relaxation.
