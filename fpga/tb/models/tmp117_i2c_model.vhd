...
begin
    ...
    process(scl, sda)
        variable bus_state : state_t := IDLE;
            ...
            case bus_state is
                ...
                when ACK_ADDRESS_SAMPLE =>
                    ...
                    if not address_ok or force_nack = '1' then
                        ...
                        bit_count := 0;
                        shift_byte := (others => '0');
                        bus_state := RECV_REGISTER;
                    end if;

                when ACK_REGISTER_DRIVE =>
                    if force_nack = '0' then sda_low <= '1'; else sda_low <= '0'; end if;
                    bus_state := ACK_REGISTER_SAMPLE;

                when ACK_REGISTER_SAMPLE =>
                    sda_low <= '0';
                    bus_state := IDLE; -- controller follows with repeated START.

                when SEND_DATA =>
                    if send_bit = 0 then
                        sda_low <= '0';
                        bus_state := WAIT_MASTER_ACK;
                    else
                        send_bit := send_bit - 1;
