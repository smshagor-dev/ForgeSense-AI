...
- Dual-signed maintenance-authority transition workflow with old-authority continuity signature, new-key proof-of-possession signature over the same deterministic payload, source/sdkconfig/rebuilt-image SHA-256 binding, unchanged calibration-state enforcement, post-install read-only fingerprint verification, explicit audit-ledger `authority_transition` continuity, and strict no-private-key/no-runtime-key-update/no-firmware-write tooling authority.
- Per-device calibration audit ledger with fresh-device genesis, append-structured SHA-256 entry chaining, metadata/genesis binding, exact active-record and signer-fingerprint continuity checks before physical maintenance operations, committed write/recovery/reboot evidence binding, replay/tamper detection, explicit reconciliation from retained evidence after host interruption, and strict documentation that the ledger is tamper-evident rather than immutable or digitally signed storage.
