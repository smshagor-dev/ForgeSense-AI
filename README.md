...
- fixed-memory mixed sensor/status stream decoding with independent sequence freshness checks;
- invalid-sensor feature-window reset;
- host-tested edge inference component;
- ML frame encoding and UART transmission;
- link-loss detection;
- authoritative FPGA status-transition and ML warning/critical event records;
...
`make dashboard` starts the dependency-free local read-only monitor on `127.0.0.1:8765` using the deterministic reference feed. FPGA status remains the displayed machine-state authority and no actuator routes are exposed.
